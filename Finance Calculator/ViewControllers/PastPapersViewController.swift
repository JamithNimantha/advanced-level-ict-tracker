...
class PastPapersViewController: UIViewController {
    ...
    var pastPapersData : PastPapersData?
    ...
    override func viewWillAppear(_ animated: Bool) {
        navigationController?.navigationBar.topItem!.title = NSLocalizedString("pastPapers", comment: "")
    ...
    func storeCalculatedData()
    {
        ...
        _ = PastPapersData.init(pv: presentValue, fv: futureValue, interest: interestRate, noOfPayments: noOfPayments, noOfCompounds: noOfCompounds, payment: payment, pmtMadeAt: pmtMadeAt, id: id, insertIntoManagedObjectContext: self.context)
    ...
    func viewStoredData()
    {
        let request = NSFetchRequest<NSFetchRequestResult>(entityName:"PastPapersData")
        do
        {
            let pastPaperData = try self.context?.fetch(request) as! [PastPapersData]
            if(pastPaperData.count > 0 ){
            
                pastPaperData.forEach {pastPaperDataObj in
                
                     print("--------------------------")
                     print("Id",pastPaperDataObj.id ?? "")
                     print("Present Value",pastPaperDataObj.presentValue)
                     print("Future Value",pastPaperDataObj.futureValue)
                     print("Interest Value",pastPaperDataObj.interest)
                     print("Compunds perYear",pastPaperDataObj.compundsPerYear)
                     print("Payments PerYear",pastPaperDataObj.paymentsPerYear)
                     print("Payment",pastPaperDataObj.payment)
                     print("Payment Made At",pastPaperDataObj.paymentMadeAt)
                }
